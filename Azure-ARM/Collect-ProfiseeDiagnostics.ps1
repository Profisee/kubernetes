...
.DESCRIPTION
    ...
    the run. SQL server/database are auto-discovered by environment variable, provided
    via parameters, or through integrated security; plaintext credentials are never
    written to disk (config files are copied with their values still encrypted).
...
Write-Log "Options       : ProcessScope=$ProcessScope HoursBack=$HoursBack MaxLogAgeDays=$MaxLogAgeDays IncludeDumps=$IncludeDumps SkipSql=$SkipSql SkipConfigs=$SkipConfigs"
    ...
    try {
        foreach ($line in (& $appcmd list site 2>$null)) {
            if ($line -match '^SITE\s+"([^"]+)"\s+\(id:(\d+)') {
                ...
                $logDir = (& $appcmd list site "$name" /text:logFile.directory 2>$null) | Select-Object -First 1
...
Invoke-Collector 'IIS runtime state' {
    ...
    if ($scoped) {
        ...
        $wp = foreach ($pool in $script:OurPools) { & $appcmd list wp /apppool:"$pool" 2>&1 }
        $wp | Out-File (Join-Path $dirs.Proc 'iis_workerprocesses.txt') -Encoding UTF8
        $ap = foreach ($pool in $script:OurPools) { & $appcmd list apppool "$pool" 2>&1 }
        $ap | Out-File (Join-Path $dirs.Proc 'iis_apppools.txt') -Encoding UTF8
        # Currently-executing requests per pool - the smoking gun for an in-progress hang.
        $rq = foreach ($pool in $script:OurPools) { & $appcmd list request /apppool:"$pool" /elapsed:1000 2>&1 }
        $rq | Out-File (Join-Path $dirs.Proc 'iis_active_requests.txt') -Encoding UTF8
    } else {
        & $appcmd list wp                    2>&1 | Out-File (Join-Path $dirs.Proc 'iis_workerprocesses.txt') -Encoding UTF8
        & $appcmd list apppool               2>&1 | Out-File (Join-Path $dirs.Proc 'iis_apppools.txt')       -Encoding UTF8
        & $appcmd list request /elapsed:1000 2>&1 | Out-File (Join-Path $dirs.Proc 'iis_active_requests.txt') -Encoding UTF8
    }
    # Site inventory is small and useful for context regardless of scope.
    & $appcmd list site 2>&1 | Out-File (Join-Path $dirs.Proc 'iis_sites.txt') -Encoding UTF8
    ...
    try {
        $rows = foreach ($line in (& $appcmd list apppool 2>$null)) {
            ...
            $auto  = (& $appcmd list apppool "$name" /text:autoStart 2>$null) | Select-Object -First 1
            [pscustomobject]@{
                AppPool        = $name
                State          = $state
    ...
    try {
        ...
        if ($_.Exception.Response) {
            ...
            try {
                ...
                $out.BodyPreview = $sr.ReadToEnd(); $sr.Close()
            } catch {}
        }
        ...
        $sw.Stop(); $out.LatencyMs = [int]$sw.Elapsed.TotalMilliseconds
    }
    [pscustomobject]$out
}
...
Invoke-Collector 'Live health endpoints' {
    ...
    $results = foreach ($u in ($urls | Sort-Object)) { Invoke-HealthProbe -Url $u }
    $results | Select-Object Code, LatencyMs, Url, Error |
        Format-Table -AutoSize | Out-File (Join-Path $dirs.Net 'health_summary.txt') -Encoding UTF8 -Width 4096
    $results | ConvertTo-Json -Depth 4 | Out-File (Join-Path $dirs.Net 'health_detail.json') -Encoding UTF8
    Write-Log ("  probed {0} health endpoints" -f $results.Count)
}
...
Invoke-Collector 'Windows event logs' {
    ...
    foreach ($log in @('Application','System')) {
        ...
        & wevtutil epl $log "$evtx" /q:"$q" /ow:true 2>&1 | Out-Null
    }
...
Invoke-Collector 'Performance counters' {
    ...
    try {
        (Get-Counter -Counter $counters -SampleInterval 1 -MaxSamples 3 -ErrorAction Stop).CounterSamples |
