...
if [ -z "$subscriptionContributor" ]; then
	...
	if [ "$UPDATEDNS" = "Yes" ]; then
		...
		if [ -z "$dnsznContributor" ]; then
			err="Role is NOT assigned. Exiting with error. Please assign the DNS Zone Contributor role to the Deployment Managed Identity for the DNS zone you want updated. Please visit https://support.profisee.com/wikis/2023_r1_support/planning_your_managed_identity_configuration for more information."
			echo $err
			set_resultAndReturn;
		else
