...
if [ "$USELETSENCRYPT" = "Yes" ]; then
	...
	helm install -n profisee nginx ingress-nginx/ingress-nginx --values nginxSettings.yaml --version="4.11.5" --set controller.service.loadBalancerIP=$nginxip --set controller.service.appProtocol=false --set controller.service.annotations."service\.beta\.kubernetes\.io/azure-dns-label-name"=$DNSHOSTNAME;
else
	...
	helm install -n profisee nginx ingress-nginx/ingress-nginx --values nginxSettings.yaml --version="4.11.5" --set controller.service.loadBalancerIP=$nginxip --set controller.service.appProtocol=false
fi
